...
public abstract class PlayerPathNavigation {
    ...
    public PlayerPathNavigation(Player player, Level pLevel) {
        this.player = player;
        this.level = pLevel;
        int i = 100; //followRange
    ...
    protected abstract PlayerPathFinder createPathFinder(int pMaxVisitedNodes);
...
//        return this.createPath(pTargets, pRegionOffset, pOffsetUpward, pAccuracy, (float)this.player.getAttributeValue(Attributes.FOLLOW_RANGE));
        return this.createPath(pTargets, pRegionOffset, pOffsetUpward, pAccuracy, 100);
    }
    ...
    public void tick() {
        ...
        if (!this.isDone()) {
            if (this.canUpdatePath()) {
                ...
                Vec3 vec3 = this.getTempMobPos();
                Vec3 vec31 = this.path.getNextEntityPos(this.player);
                if (vec3.y > vec31.y && !this.player.onGround() && Mth.floor(vec3.x) == Mth.floor(vec31.x) && Mth.floor(vec3.z) == Mth.floor(vec31.z)) {
                    this.path.advance();
                }
            }
...
//        this.maxDistanceToWaypoint = this.player.getBbWidth() > 0.75F ? this.player.getBbWidth() / 2.0F : 0.75F - this.player.getBbWidth() / 2.0F;
    ...
    private boolean shouldTargetNextNodeInDirection(Vec3 pVec) {
        if (this.path.getNextNodeIndex() + 1 >= this.path.getNodeCount()) {
            ...
            if (!pVec.closerThan(vec3, 2.0D)) {
                return false;
            } else if (this.canMoveDirectly(pVec, this.path.getNextEntityPos(this.player))) {
                return true;
            } else {
    ...
    protected boolean isInLiquid() {
        return this.player.isInWaterOrBubble() || this.player.isInLava();
    }
    ...
    public PlayerNodeEvaluator getNodeEvaluator() {
