...
public class PlayerWalkNodeEvaluator extends PlayerNodeEvaluator {
    ...
    public void prepare(PathNavigationRegion pLevel, Player pMob) {
...
//        this.mob.onPathfindingDone();
    ...
    protected boolean canStartAt(BlockPos pPos) {
        ...
        return blockpathtypes != BlockPathTypes.OPEN && getPathfindingMalus(blockpathtypes) >= 0.0F;
    }
    ...
    public int getNeighbors(Node[] pOutputArray, Node pNode) {
        ...
        if (getPathfindingMalus(blockpathtypes) >= 0.0F && blockpathtypes1 != BlockPathTypes.STICKY_HONEY) {
            ...
            if (blockpathtypes != BlockPathTypes.WALKABLE && (!this.isAmphibious() || blockpathtypes != BlockPathTypes.WATER)) {
                ...
                if (!this.isAmphibious() && blockpathtypes == BlockPathTypes.WATER && !this.canFloat()) {
                    ...
                    while (pY > this.mob.level().getMinBuildHeight()) {
                        ...
                        node = this.getNodeAndUpdateCostToMax(pX, pY, pZ, blockpathtypes, getPathfindingMalus(blockpathtypes));
                    }
                }
                ...
                if (blockpathtypes == BlockPathTypes.OPEN) {
                    ...
                    while (blockpathtypes == BlockPathTypes.OPEN) {
                        ...
                        blockpathtypes = this.getCachedBlockType(this.mob, pX, pY, pZ);
                        f = getPathfindingMalus(blockpathtypes);
    ...
    public BlockPathTypes getBlockPathType(BlockGetter pLevel, int pX, int pY, int pZ, Player pMob) {
        ...
        if (enumset.contains(BlockPathTypes.FENCE)) {
            ...
            for (BlockPathTypes blockpathtypes2 : enumset) {
                ...
                if (getPathfindingMalus(blockpathtypes2) >= getPathfindingMalus(blockpathtypes1)) {
                    blockpathtypes1 = blockpathtypes2;
                }
            }

            return blockpathtypes == BlockPathTypes.OPEN && getPathfindingMalus(blockpathtypes1) == 0.0F && this.entityWidth <= 1 ? BlockPathTypes.OPEN : blockpathtypes1;
        }
    }
     ...
     */
    protected BlockPathTypes getBlockPathType(Player pEntityliving, BlockPos pPos) {
     ...
     */
    protected BlockPathTypes getCachedBlockType(Player pEntity, int pX, int pY, int pZ) {
        return this.pathTypesByPosCache.computeIfAbsent(BlockPos.asLong(pX, pY, pZ), (p_265015_) -> {
            return this.getBlockPathType(pEntity.level(), pX, pY, pZ, pEntity);
        });
    }
...
//        Float f = pathfindingMalus.get(pNodeType);
//        return f == null ? pNodeType.getMalus() : f;
        return pNodeType.getMalus();
    }
}
